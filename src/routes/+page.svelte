<script lang="ts">
  ...
  function matchesLeftCtrl(event: KeyboardEvent) {
    return event.code === 'ControlLeft' || (event.key === 'Control' && event.location === KeyboardEvent.DOM_KEY_LOCATION_LEFT);
  }
  ...
  function matchesLeftShift(event: KeyboardEvent) {
    return event.code === 'ShiftLeft' || (event.key === 'Shift' && event.location === KeyboardEvent.DOM_KEY_LOCATION_LEFT);
  }

  function matchesRightShift(event: KeyboardEvent) {
    return event.code === 'ShiftRight' || (event.key === 'Shift' && event.location === KeyboardEvent.DOM_KEY_LOCATION_RIGHT);
  }

  function matchesLeftBracketSide(event: KeyboardEvent) {
    return event.code === 'BracketLeft' || event.key === '[' || event.key === '{' || event.key === '<' || event.code === 'Comma';
  }

  function matchesRightBracketSide(event: KeyboardEvent) {
    return event.code === 'BracketRight' || event.key === ']' || event.key === '}' || event.key === '>' || event.code === 'Period';
  }

  function getSideFromEvent(event: KeyboardEvent): Side | null {
    const ditPhysical = matchesLeftCtrl(event) || matchesLeftShift(event) || matchesLeftBracketSide(event);
    const dahPhysical = matchesRightCtrl(event) || matchesRightShift(event) || matchesRightBracketSide(event);
...
<div class="min-h-screen bg-[#07080b] text-white antialiased">
  <main class="flex min-h-screen max-w-[90%] flex-col px-6 py-12 sm:px-10 lg:px-14 mx-auto">
    <header class="mb-16">
    ...
    <section class="w-full">
      <div class="grid grid-cols-1 gap-8 sm:grid-cols-3">
        <label class="space-y-3">
          ...
          <select
            bind:value={mode}
            class="w-full rounded-md border border-white/15 bg-white/90 px-3 py-2 text-lg text-black outline-none transition focus:border-white/40"
          >
        ...
        <label class="space-y-3">
          ...
          <input bind:value={toneHz} min="300" max="1200" step="10" type="range" class="w-full accent-[#79c6f2]" />
          <div class="text-center text-sm text-white/55">{toneHz} hz</div>
        </label>
      </div>

      <div class="mt-14 rounded-[2rem] border border-white/6 bg-[linear-gradient(90deg,rgba(46,58,49,0.72),rgba(34,44,42,0.88))] p-7 shadow-[0_0_0_1px_rgba(255,255,255,0.02)] sm:p-8">
        <div class="mb-5 flex flex-wrap items-start justify-between gap-4">
          <div>
            <div class="text-[1.05rem] text-white/95">Decoded Text</div>
            <div class="mt-2 text-sm text-white/45">
              {ditLabel}: Left Ctrl / Left Shift / [ / &lt; &nbsp;·&nbsp; {dahLabel}: Right Ctrl / Right Shift / ] / &gt;
            </div>
          </div>
        ...
        <div class="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-white/8 pt-4 text-sm text-white/50">
    ...
    <div class="text-white/35 mt-2 text-center">Made by Iona a SWL (M7DXZ in a few days)</div>
  </main>
</div>